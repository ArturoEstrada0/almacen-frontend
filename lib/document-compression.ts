...
export function isValidDocumentFile(
  ...
  return (
    allowedExtensions.includes(getFileExtension(file.name)) && file.size > 0
  );
}

function shouldZipFile(
  file: File,
  options: Required<DocumentCompressionOptions>,
): boolean {
...
export async function compressDocument(
  ...
  const merged = {
    ...DEFAULT_OPTIONS,
    ...options,
  } as Required<DocumentCompressionOptions>;
  ...
  const zipBlob = new Blob([zippedBytes as unknown as ArrayBuffer], {
