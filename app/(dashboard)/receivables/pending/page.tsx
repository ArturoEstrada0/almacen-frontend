...
export default function PendingReceivablesPage() {
  ...
  const customerId = searchParams ? searchParams.get('customerId') : null
  const [selectedCustomerId, setSelectedCustomerId] = useState(customerId || "all")
  ...
  const fetchData = async (range?: DateRange, customerFilterId?: string | null) => {
    setLoading(true)
    try {
      const qs = new URLSearchParams()
      const effectiveCustomerId = customerFilterId && customerFilterId !== "all" ? customerFilterId : null
      if (effectiveCustomerId) qs.set('customerId', effectiveCustomerId)
      if (range?.from) qs.set('startDate', format(range.from, 'yyyy-MM-dd'))
      if (range?.to) qs.set('endDate', format(range.to, 'yyyy-MM-dd'))

      const url = `${API_ENDPOINTS.customers.pendingReceivables()}${qs.toString() ? `?${qs.toString()}` : ''}`
      const res = await ApiClient.get<PendingRecord[]>(url)
      setRecords(res || [])
    } catch (err) {
      console.error('Failed to load pending receivables', err)
      setRecords([])
    } finally {
      setLoading(false)
    }
  ...
  const fetchCustomers = async () => {
    ...
    try {
      const res = await ApiClient.get<CustomerFilterOption[]>(API_ENDPOINTS.customers.list())
      setCustomers(res || [])
    } catch (err) {
  ...
  useEffect(() => {
    setSelectedCustomerId(customerId || "all")
    fetchData(undefined, customerId || "all")
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold">Facturas pendientes (clientes)</h2>
        <div className="flex items-center gap-2">
          ...
          <ComboBox
            ...
            onChange={(value) => setSelectedCustomerId(value || "all")}
          ...
          <Button onClick={() => fetchData(dateRange, selectedCustomerId)} disabled={loading}>{loading ? 'Cargando...' : 'Filtrar'}</Button>
        </div>
      </div>
